...
Two limits meet here. Chroma turns a multi-value filter into SQLite parameters
and the limit is reached by the number of *matching chunks*, not the number of
ids: measured on this corpus, 10 ids matching 11,125 chunks was fine and 20 ids
was not, and a manual holds anywhere from 328 to several thousand chunks. And
the previous export built one JSON object out of every chunk, which the OOM
killer ended at 504,346 of them.
"""
...
class FakeStore:
    """A vector store that refuses a filter matching too many chunks."""
    ...
    def scroll(self, where, *, with_document=True, with_embedding=False, **kwargs):
        ...
        for mid in ids:
            for i in range(self.per_manual):
                embedding = None
                if self.embeddings and with_embedding:
                    # The Chroma backend converts numpy rows to lists; this
                    # stands in for one that has already done so.
                    embedding = np.array([0.1, 0.2], dtype=np.float32).tolist()
                yield Chunk(
                    id=f"{mid}_{i}",
                    document=f"text {mid} {i}",
                    metadata={"manual_id": mid},
                    embedding=embedding,
                )
...
def written(tmp_path, col, ids):
    ...
    with open(path, "w", encoding="utf-8") as out:
        count = _write_chunks(col, ids, out)
    with open(path, encoding="utf-8") as handle:
        return count, list(_read_chunks(handle))
...
def test_a_corpus_sized_export_asks_for_one_manual_at_a_time(tmp_path):
    ...
    col = FakeStore(per_manual=5000, chunk_limit=11000)
    _write_chunks(col, [f"m{i}" for i in range(369)], io.StringIO())
    assert set(col.queries) == {1}
    assert len(col.queries) == 369


def test_every_chunk_is_written_exactly_once(tmp_path):
    count, chunks = written(tmp_path, FakeStore(per_manual=3),
...
def test_a_chunk_keeps_its_four_fields_together(tmp_path):
    _, chunks = written(tmp_path, FakeStore(), ["m0", "m1"])
    for c in chunks:
...
def test_embeddings_survive_the_round_trip_as_plain_lists(tmp_path):
    _, chunks = written(tmp_path, FakeStore(), ["m0"])
    assert chunks[0]["embedding"] == pytest.approx([0.1, 0.2], abs=1e-6)


def test_one_line_per_chunk():
    out = io.StringIO()
    _write_chunks(FakeStore(per_manual=3), ["m0", "m1"], out)
...
def test_a_chunk_without_an_embedding_is_kept_with_a_null(tmp_path):
    ...
    _, chunks = written(tmp_path, FakeStore(embeddings=False), ["m0"])
    assert [c["embedding"] for c in chunks] == [None, None]


def test_no_chunks_at_all_writes_an_empty_file(tmp_path):
    count, chunks = written(tmp_path, FakeStore(per_manual=0), ["m0", "m1"])
    assert count == 0
    assert chunks == []
...
def test_it_handles_any_number_of_manuals(tmp_path, n):
    count, chunks = written(tmp_path, FakeStore(), [f"m{i}" for i in range(n)])
...
def test_blank_lines_in_the_chunk_file_are_skipped():
    stream = io.StringIO('{"id": "a"}\n\n{"id": "b"}\n')
    assert [c["id"] for c in _read_chunks(stream)] == ["a", "b"]
