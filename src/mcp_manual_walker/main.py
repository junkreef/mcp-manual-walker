    ...
    try:
        ...
        for pdf_path in scan_pdfs(pdf_root_path):
            ...
            if relative_path not in db_paths or db_manuals[relative_path].file_hash != file_hash:
                ...
                db.add(new_manual)
                db.flush()  # Flush to get the new_manual.id
                ...
                for bm_data in pdf_data["bookmarks"]:
                    ...
                    db.add(bookmark)
                    db.flush()  # Flush to get bookmark.id for parent tracking
        ...
        logger.error(f"An error occurred during database synchronization: {e}", exc_info=True)
        db.rollback()
    finally:
        db.close()
...
def list_manuals() -> list[dict[str, Any]]:
    """
    Returns a list of all available manuals, including their ID, filename, and document title.
    """
    db: Session = SessionLocal()
    try:
...
def _build_toc(bookmarks: list[Bookmark]) -> list[dict[str, Any]]:
...
@app.tool()
def get_manual_metadata(manual_id: str) -> dict[str, Any]:
...
def get_markdown_content(bookmark_id: str) -> str:
    ...
    try:
        ...
        logger.info(
            f"Cache miss. Extracting pages {start_page}-{end_page or 'end'} "
            f"from '{pdf_path.name}' for bookmark '{bookmark.title}'."
        )
        raw_text = extract_text_from_page_range(pdf_path, start_page, end_page)
        if not raw_text:
            return f"Error: Could not extract text for bookmark '{bookmark.title}' (ID: {bookmark_id})."
        ...
        logger.error(f"Error getting content for bookmark_id '{bookmark_id}': {e}", exc_info=True)
