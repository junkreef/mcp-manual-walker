...
def command_list(args):
    ...
    try:
        ...
        if args.json:
            print(json.dumps([manual_to_dict(m) for m in manuals], indent=2, default=str))
        else:
            print(f"{'ID':<38} | {'Pages':<6} | {'Path'}")
            print("-" * 80)
            for manual in manuals:
                print(f"{manual.id:<38} | {manual.page_count:<6} | {manual.relative_path}")
...
def command_import(args):
    ...
    try:
        from mcp_manual_walker.builder import get_embedding_function
        logger.info("Loading embedding function...")
        ef = get_embedding_function()
    except ImportError:
        # Fallback if dependencies are missing, but ideally they shouldn't be.
        logger.warning("Could not load embedding function. Falling back to default (if possible) or error.")
        # If we cannot load it, Chroma might complain if we try to get_or_create.
        # But we only need to add generated embeddings, so maybe we can pass None?
        # However, to be consistent with the collection config, we should use it.
        # Assuming the environment has the same deps (sentence-transformers).
        ef = None
    ...
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            logger.info(f"Importing export from {manifest['created_at']}, target: {manifest['target']}")
        ...
        for manual_dict in sqlite_data:
            ...
            if existing:
                logger.warning(f"Manual {manual_dict['relative_path']} (ID: {manual_dict['id']}) already exists. Skipping.")
                skipped_count += 1
                continue
            ...
            manual = Manual(
                ...
                updated_at=datetime.fromisoformat(manual_dict["updated_at"]) if manual_dict["updated_at"] else None
            )
            session.add(manual)
        ...
        session.commit()
        logger.info(f"SQLite Import: {imported_count} imported, {skipped_count} skipped.")
...
def main():
    ...
    parser_build.add_argument("--pdf_dir", type=str, required=True, help="Directory containing PDF files")
    parser_build.add_argument("--save-markdown", action="store_true", help="Save intermediate Markdown files")
    parser_build.add_argument("--reset", action="store_true", help="Reset database before building")
    ...
    parser_export.add_argument("--target", type=str, required=True, help="Target file or directory path relative to PDF root")
    parser_export.add_argument("--output", type=str, required=True, help="Output ZIP file path")
    parser_export.set_defaults(func=command_export)

    # Import Command
    parser_import = subparsers.add_parser("import", help="Import data")
    parser_import.add_argument("--input", type=str, required=True, help="Input ZIP file path")
    parser_import.set_defaults(func=command_import)

    # Delete Command
    parser_delete = subparsers.add_parser("delete", help="Delete data")
    parser_delete.add_argument("--target", type=str, required=True, help="Target file or directory path relative to PDF root")
